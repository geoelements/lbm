...
TEST_CASE("D2Q9 Lattice parameters are checked", "[LBM][Lattice][D2Q9]") {
  ...
  const auto d2q9 = lbm::Lattice<DIM, NQ>();
  ...
  SECTION("D2Q9 lattice weights sum to 1") {
    lbm::Real sum = 0;
    for (unsigned i = 0; i < NQ; ++i) sum += d2q9.weights.at(i);
  ...
  SECTION("D2Q9 lattice velocities sum to zero") {
    std::array<lbm::Real, DIM> sum{0};
    for (unsigned i = 0; i < NQ; ++i)
      for (unsigned j = 0; j < DIM; ++j)
        sum.at(j) += d2q9.velocities.at(i).at(j);

    for (unsigned j = 0; j < DIM; ++j) REQUIRE(sum.at(j) == Approx(0.0));
  }
...
TEST_CASE("D3Q19 Lattice parameters are checked", "[LBM][Lattice][D3Q19]") {
  ...
  const auto d3q19 = lbm::Lattice<DIM, NQ>();
  ...
  SECTION("D3Q19 lattice weights sum to 1") {
    lbm::Real sum = 0;
    for (unsigned i = 0; i < NQ; ++i) sum += d3q19.weights.at(i);
    REQUIRE(sum == Approx(1.0));
  }
...
TEST_CASE("D3Q27 Lattice parameters are checked", "[LBM][Lattice][D3Q27]") {
  ...
  const auto d3q27 = lbm::Lattice<DIM, NQ>();
  ...
  SECTION("D3Q27 lattice weights sum to 1") {
    lbm::Real sum = 0;
    for (unsigned i = 0; i < NQ; ++i) sum += d3q27.weights.at(i);
    REQUIRE(sum == Approx(1.0));
  }
  ...
  SECTION("D3Q27 lattice velocities sum to zero") {
    std::array<lbm::Real, DIM> sum{0};
    for (unsigned i = 0; i < NQ; ++i)
      for (unsigned j = 0; j < DIM; ++j)
        sum.at(j) += d3q27.velocities.at(i).at(j);
